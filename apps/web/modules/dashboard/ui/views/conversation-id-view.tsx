...
const ConversationIdView = ({ conversationId }: { conversationId: Id<"conversations"> }) => {
    ...
    const messages = useThreadMessages(
        ...
        conversation?.threadId && conversation.contactSessionId
            ? { threadId: conversation.threadId, contactSessionId: conversation.contactSessionId }
            : "skip",
        { initialNumItems: 10 }
    );
    ...
    return (
        <div className='flex h-full flex-col bg-muted'>
            <header className='flex items-center justify-between border-b bg-background p-2.5'>
                ...
                {conversation && (
                    <ConversationStatusButton status={conversation?.status} onClick={handleToggleStatus} disabled={isUpdatingStatus} />
                )}
            </header>
            ...
            <div className='p-2'>
                <Form {...form}>
                    <AIInput onSubmit={form.handleSubmit(onSubmit)}>
                        <FormField
                            ...
                            render={({ field }) => (
                                <AIInputTextarea
                                    ...
                                    placeholder={
                                        conversation?.status === "resolved" ? "This conversation has been resolved" :
                        ...
                        <AIInputToolbar>
                            <AIInputTools>
                                <AIInputButton disabled={conversation?.status === "resolved" || isEnhancing || !form.formState.isValid} onClick={handleEnhanceResponse}>
                                    <Wand2Icon />
            ...
            <AIConversation className='max-h-[calc(100vh-180px)]'>
                <AIConversationContent>
                    {Array.from({ length: 8 }, (_, index) => {
                        ...
                        return (
                            <div className={cn("group flex w-full items-end justify-end gap-2 py-2 [&>div]:max-w-[80%]", isUser ? "is-user" : "is-assistant flex-row-reverse")}
                                key={index}>
